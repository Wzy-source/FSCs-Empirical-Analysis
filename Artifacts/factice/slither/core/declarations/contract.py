...
class Contract(SourceMapping):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, compilation_unit: "SlitherCompilationUnit", scope: "FileScope") -> None:
        ...
        # factory related properties
        self._is_factory_contract: Optional[bool] = None
        self._is_create_factory_contract: Optional[bool] = None
        self._is_create2_factory_contract: Optional[bool] = None
    ...
    def is_factory(self) -> bool:
        """
        在所有函数中检查是否有"new contract行为"，如果有，设置self._is_factory = True
        """
        if self._is_factory_contract is None:
            self._is_factory_contract = False
            for fn in self.functions:
                # 遍历合约中所有非构造函数，如果有一个函数是工厂函数，则为工厂合约
                if fn.is_factory_function:
                    self._is_factory_contract = True
                    break
        return self._is_factory_contract

    @property
    def factory_functions(self) -> List["FunctionContract"]:
        factory_list = []
        for fn in self.functions:
            if not fn.is_constructor:
                if fn.is_factory_function:
                    factory_list.append(fn)
        return factory_list
