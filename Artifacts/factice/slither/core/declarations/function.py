...
class Function(SourceMapping, metaclass=ABCMeta):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, compilation_unit: "SlitherCompilationUnit") -> None:
        ...
        # factory-related Property
        self._is_facfn: Optional[bool] = None
        self._is_create_facfn: Optional[bool] = None
        self._is_create2_facfn: Optional[bool] = None
        self._create_factory_nodes: Optional[List[Tuple["Node", Union[str, Dict]]]] = None
        self._create2_factory_nodes: Optional[List[Tuple["Node", Union[str, Dict]]]] = None
    ...
    @property
    def is_factory_function(self) -> bool:

        from slither.slithir.operations import NewContract
        from slither.core.cfg.node import NodeType
        # 相当于该函数只执行一次
        if self._is_facfn is None:
            self._is_facfn = False
            self._create_factory_nodes = []
            self._create2_factory_nodes = []
            # 遍历函数的所有nodes，判断是否有new Contract相关的操作
            for node in self.nodes:
                for ir in node.irs:
                    # 第1种情况，直接使用new关键字创建合约，在IR中直接寻找即可
                    if isinstance(ir, NewContract):
                        # TODO 需要根据参数判断具体使用了create还是create2关键字实现的new Contract
                        self._is_facfn = True
                        break
                # 第2种情况，通过assembly块使用EVM内连汇编代码创建合约
                if node.type == NodeType.ASSEMBLY and node.inline_asm:
                    """
                    https://github.com/crytic/slither/issues/1775
                    # TODO 可能要添加其他statement节点类型
                    分为两种情况：
                    当Solidity版本>0.6时，inline_asm以AST形式呈现
                    当Solidity版本<0.6时，inline_asm以String形式呈现
                    """
                    # 第2.1种情况
                    ...
                    # 第2.2种情况,将字符串根据换行符进行分割，得到每一个statement
                        ...
                        asm_statements = _inline_asm.split("\n")
                        for statement in asm_statements:
                            if "create(" in statement:
                                self._is_create_facfn = True
                                self._create_factory_nodes.append((node, statement))
                            if "create2(" in statement:
                                self._is_create2_facfn = True
                                self._create2_factory_nodes.append((node, statement))
                        self._is_facfn = self._is_create_facfn or self._is_create2_facfn
    ...
    @property
    def create2_factory_nodes(self) -> List[Tuple["Node", Union[str, Dict]]]:
        return self._create2_factory_nodes
